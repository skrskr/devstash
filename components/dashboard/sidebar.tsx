...
const typeConfig: Record<string, { icon: LucideIcon; color: string }> = {
  snippet: { icon: Code2,     color: "text-blue-400"   },
  prompt:  { icon: Sparkles,  color: "text-purple-400" },
  command: { icon: Terminal,  color: "text-orange-400" },
  note:    { icon: FileText,  color: "text-green-400"  },
        ...
        <div className="mb-2">
          ...
          {(typesOpen || collapsed) &&
            mockItemTypes.map((type) => (
              ...
              >
                {(() => {
                  const cfg = typeConfig[type.icon];
                  if (!cfg) return null;
                  const Icon = cfg.icon;
